    ...
    public class OrderService
    {
        ...
        public OrderService()
        {
            ...
            while (flag)
            {
                ...
                Console.WriteLine("+:新增訂單,-:刪除訂單,*:查詢訂單,=:顯示所有訂單,!:排序操作,/:退出操作,%:序列化現有訂單,^:反序列化訂單");
                //+:新增訂單,-:刪除訂單,*:查詢訂單,=:顯示所有訂單,!:排序操作,/:退出操作,%:序列化現有訂單,^:反序列化訂單
        ...
        public OrderService(int a)//測試用的
        {
        ...
        public bool AddOrder(Order order)
        {
            try
            {
                if (order==null)
                {
                    Console.WriteLine("這個訂單是空的,不能增加");
                    return false;
                }
        ...
        public bool ReOrderList_ForTest(int a)
        {
            if (orderlist.Count <= 0)
            {
                return false;
        ...
        public bool Export()
        {
            ...
            using (FileStream fileStream = new FileStream("ord.xml", FileMode.Create))
            {
        ...
        public bool Import()
        {
            ...
            XmlSerializer xmlSerializer = new XmlSerializer(typeof(List<Order>));
            using (FileStream fileStream = new FileStream("ord.xml", FileMode.Open))
            {
                List<Order> list = (List<Order>)xmlSerializer.Deserialize(fileStream);
                int id2 = orderlist.Count + 1;
